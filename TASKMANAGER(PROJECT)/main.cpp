...
const string ACCOUNTS_FILE = "user_accounts.txt"; // File path for storing user accounts

void loadUserAccounts(vector<User>& userAccounts) {
    ...
    if (file.is_open()) {
        ...
        cout << "Unable to open file: " << ACCOUNTS_FILE << endl;
    }
}

void saveUserAccounts(const vector<User>& userAccounts) {
    ofstream file(ACCOUNTS_FILE);
    if (file.is_open()) {
        for (const User& user : userAccounts) {
...
void createAccount(vector<User>& userAccounts) {
    string username, password;
...
void displayMenu(int pendingTaskCount) {
        ...
        l.printLine("5. Save Task", maxLength);
        l.printLine("6. Import Task", maxLength);
        l.printLine("7. About", maxLength);
        l.printLine("8. Log out", maxLength);

        l.printBottomBorder(maxLength + 4);
        cout << "Your choice: ";

}
void saveTasksToFile(const string& filename, const LinkedList& tasks) {
    ofstream file(filename);
    if (file.is_open()) {
        for (LinkedList::Node* current = tasks.head; current != nullptr; current = current->next) {
            file << current->task->getTitle() << "," << current->task->getCategory() << "," << current->task->getDueDate() << endl;
        }
        file.close();
...
// Function prototypes
void saveTasksToFile(const string& filename, const LinkedList& tasks);
void importTasksFromFile(const string& filename, LinkedList& tasks);
int getUserChoice();
...
bool authenticateUser(const string& username, const string& password, const vector<User>& userAccounts) {
    for (const User& user : userAccounts) {
    ...
    do {
        ...
        switch (choice) {
            ...
            case 2:
                ...
                cin >> username;
                if (username == "q") {
                    cout << "Exiting User Login. Goodbye!" << endl;
                    return 0;
                }
                ...
                loggedIn = authenticateUser(username, password, userAccounts);
...
void importTasksFromFile(const string& filename, LinkedList& tasks) {
    ...
    if (file.is_open()) {
        tasks.clear(); // Clear the existing tasks before importing

        string line;
        while (getline(file, line)) {
            istringstream iss(line);
            string title, category, dueDate;
            if (iss >> title >> category >> dueDate) {
                Task* newTask = new Task(title, category, dueDate);
                tasks.addTask(newTask);
            }
        }

        cout << "Tasks imported from file successfully!" << endl;
        file.close();

        // Continue to the task menu
        int choice;
        do {
            displayMenu(tasks.pendingTaskCount);
            choice = getUserChoice();
